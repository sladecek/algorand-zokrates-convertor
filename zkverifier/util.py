...
def fullyCompileSmartSignature(client: AlgodClient, contract: Expr) -> bytes:
    teal = compileTeal(contract, mode=Mode.Signature, version=5)
    teal = teal.replace("pragma version 5", "pragma version 9")
    print(teal)
    response = client.compile(teal)
    print(response)
    return b64decode(response["result"])
